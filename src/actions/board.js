...
export const LOAD_BOARD_MEMBERS = "LOAD_BOARD_MEMBERS";
...
const receiveBoardMembers = (boardMembers) => {
    return {
        type: LOAD_BOARD_MEMBERS,
        boardMembers
    }
}
...
export const loadBoard = (uid) => dispatch => {
    ...
    myFirebase.database().ref('/board/' + uid).once('value').then(function (snapshot) {
        const board = {
            ...
            backgroundPhoto: snapshot.val().backgroundPhoto
        }
...
export const loadBoardMembers = (uid) => dispatch => {
    myFirebase.database().ref('/boards/' + uid + '/members/').once('value').then(function (snapshot) {
        var memberIDList = [];
        for(var key in snapshot.val()) { 
            memberIDList.push(snapshot.val()[key]['uid']) 
        }
        
        dispatch(receiveBoardMembers(memberIDList));
    })
}
